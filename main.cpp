...
thread column_thread(column_check,sudoku);
thread row_thread(row_check,sudoku);
thread subgrid_thread(subgrid_check,sudoku);
...
bool check = col && row && subgrid;

cout << check << endl;

if (check){
    cout<<"The sudoku is Valid";
}else{
    cout<< "The sudoku is Invalid";
}
